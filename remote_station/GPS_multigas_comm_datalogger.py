...
def read_gps_all(gps_ser) :
    """
    Reads the whole information given by the GPS module
    """
    ...
    with gps_ser as ser :
        ...
        while not start_by_GGA :
            ...
            if data.find(b'GGA') > 0 : #We check if the line starts by GGA to parse it
                ...
                if msg.timestamp == None :
                    yield None, " ,{0} {1} ,{2} {3} ,{4} {5}\n".format(msg.latitude, msg.lat_dir, msg.longitude, msg.lon_dir, msg.altitude, msg.altitude_units)
                
                else :
                    yield msg.timestamp.replace(microsecond=0, tzinfo=None), " ,{0} {1} ,{2} {3} ,{4} {5}\n".format(msg.latitude, msg.lat_dir, msg.longitude, msg.lon_dir, msg.altitude, msg.altitude_units)
...
def get_single_measurement(ser, first_iteration, measure_number):
    '''
    Get a single measurement from the sensors on the board.
    '''

    #pool the update register. The result is 1 if a new data is available on the board
    data = multisensor.read_UPDATE(system, ser, multisensor_address)
    ...
    #the header
    data = multisensor.read_MEASUREMENTS(system, ser, multisensor_address)
    #save the first iteration multisensor time stamp
    # if first_iteration:
    #     time_stamp0=int.from_bytes(new_data[0:4], byteorder='big', signed=False)
    ...
    for i in range (1, measure_number+1):
        ...
        #measured_value = bytes(data[(offset-4) : offset]) #Lucie's test to understand why there is an offset like that
        # unpack the 4 bytes as float number and force big-endian
        [measured_value] = struct.unpack('>f', measured_value)
        # Adding sensor colums
...
def write_single_measurement(gps_ser, sensor_ser, first_iteration, measure_number) :
    ...
    for val1, val2 in zip(read_gps_all(gps_ser), get_single_measurement(sensor_ser, first_iteration, measure_number)) :
        print_and_write(file, str(val1[0])+val2+val1[1])
