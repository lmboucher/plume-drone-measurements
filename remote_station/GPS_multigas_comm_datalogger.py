...
serial_port_names_comm = ['/dev/ttyUSB0', '/dev/ttyUSB1']
...
number_of_points=-1
#file name where to save the data log. Different for each acquisition. Takes computer time, true only when connected to Internet
now = datetime.datetime.now()
log_file_name="/home/tecnosense/lucie_board_test/data_risultati_"+now.strftime("%H_%M_%S_%f")+".txt"
...
def read_gps_info(gps_ser) :
    ...
    with gps_ser as serialPort :
        ...
        while not start_by_GGA : #Until our line is not starting by GGA, we read another one
            ...
            if data.find(b'GGA') == 3 : #check for bytes-like objects starting by GGA
                dollar = data.find(b'$')
                msg = pynmea2.parse(data[dollar:].decode()) #returns a sentence GGA(...) with different information that we take and structure on next line
                ...
                if msg.timestamp == None :
                    ...
                    return str(msg.timestamp.replace(microsecond=0, tzinfo=None)), pos
            else :
...
def read_gps_timestamp(gps_ser) :
    ...
    with gps_ser as ser :
        ...
        while not start_by_GGA :
            ...
            if data.find(b'GGA') == 3 : #We check if the line starts by GGA to parse it
                dollar = data.find(b'$')
                msg = pynmea2.parse(data[dollar:].decode())
                start_by_GGA = True
...
def open_communication_port() :
    ...
    for port_name in serial_port_names_comm :
        
        try :
            #By default no parity, 8 data bits, 1 stop bit, so no need to set them
            comm_ser = serial.Serial(port_name, 57600)
    ...
    plugged_in = False #If no valid port has been found
...
def write_single_measurement(gps_ser, sensor_ser, first_iteration, measure_number) :
    ...
    # Get GPS info
    gps_data = read_gps_info(gps_ser)
    
    #Adding GPS time column
    table_line = gps_data[0]
...
if __name__ == "__main__":
    ...
    # Opening the multisensor board port
    sensor_ser = open_sensor_port()
    
    # Opening the GPS port
    gps_ser = serial.Serial(serial_port_name_GPS, 9600, timeout=0.1)
    
    # Opening the communication port
    open_comm_port = open_communication_port()
    plugged_in = open_comm_port[0]
    comm_ser = open_comm_port[1]
